...
public final class SkullItemStackBuilder extends ItemStackBuilder {
  ...
  public SkullItemStackBuilder profile(GameProfile profile) {
    ...
    if (!this.getItemStack().getType().equals(Material.PLAYER_HEAD)
        && !this.getItemStack().getType().equals(Material.PLAYER_WALL_HEAD)) {
      return this;
    }

    try {
      SkullItemStackBuilder.skullOwnerField.get().set(this.getItemMeta(), profile);
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public SkullItemStackBuilder texture(String textureUrlBase64) {
